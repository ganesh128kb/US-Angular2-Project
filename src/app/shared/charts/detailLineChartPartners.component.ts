//Angular2 Specifics
import { Component, ViewContainerRef, OnInit, OnDestroy, ViewEncapsulation } from '@angular/core';
...
@Component({
    selector: 'detail-line-chart-partners',
    template: `
    <div class="no-padding text-right chart-btn"><button type="button" class="umg__button btn" (click)="exportService.download(artistService.artistName + ' - ' + 'ArtistLineChartStreamsPartner', exportService.lineChartPartnerStreamsData, exportService.csvArtistLineChartStreamsPartner)">Export Data<i class="fa fa-sign-out" aria-hidden="true" style="padding-left: 10px;"></i></button></div>
    <div id="detail-line-chart-partners" class="chart-partners"></div>`,
    styles: [`
      /deep/ #detail-line-chart-partners {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #detail-line-chart-partners > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class DetailLineChartPartnersComponent implements OnInit {
    ...
    constructor(public artistService: ArtistDetailService,
        ...
        this.filterEmit = fs.TestServiceEmit.subscribe(
            ...
            () => {
				...
				d3.select(this.elem).select(".chart-partners svg").remove();
			});
    ...
    loadData() {
        ...
        this.subscriptionLineChartPartners = this.artistService.getLineChartPartnersData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.dailyPartnerData = tempData.artist;
                ...
                this.dailyPartnersTop = d3.nest()
                    ...
                    .rollup(function(v: any) { return d3.sum(v, function(d: any) { return d.streamsUnits; }) as any })
    ...
    lineChartStreams() {
		...
		this.svg = d3.select(this.elem).select('.chart-partners')
      ...
      .attr("viewBox", "0 0 " + width + " " + height)
		...
		this.svg
			...
			.style('stroke', (d, i) => ["#1bafea", "#ddf4f7"][i])
		...
		this.svg
			...
			.style('stroke', (d, i) => ["#1bafea", "#ddf4f7"][i])
			.style('fill', (d, i) => ["#1bafea", "#ddf4f7"][i]) //, "#30c3e5", "#74d2e2", "#a6d9e8"
			...
			let circles = this.svg
				...
				.on("mouseover", d => {
					return tooltip.style("visibility", "visible")
						...
						"<div>" + "<strong>Raw: </strong>" + tooltipNumberFormat(d.streamsUnits) + "</div>" +
						"<div>" + "<strong>Adjusted: </strong>" + tooltipNumberFormat(d.adjustedStreamsUnits) + "</div>"
						);
				})
				.on("mousemove", d => {
