...
export class PieChartComponent implements OnInit {
    ...
    loadData() {
        let tempData;
        d3.select(this.elem).select(".pie-chart svg").remove();

        this.subscriptionTopFiveCountries = [];
        this.data = [];

        this.topFiveCountryData = this.artistService.getCountryData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.topFiveCountryData = tempData.artist;
                // console.log(this.topFiveCountryData);
                ...
                this.artistCountriesTopFive = this.topFiveCountryData.slice(0, 5);
                this.artistCountriesTopFive = chain({}).merge(
                    chain(this.artistCountriesTopFive)
                        .filter(tot => !!tot.territory)
                ...
                this.data = this.artistCountriesTopFive;
    ...
    pieChart() {
        ...
        this.svg = d3.select(this.elem).select(".pie-chart").append("svg")
            .attr("width", this.w)
            .attr("height", this.h)
            .style("display", "block")
            .style("margin", "0 auto")
