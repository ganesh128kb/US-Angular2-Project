...
@Component({
    selector: 'track-detail-line-chart-overview',
    template: `<div class="error"></div>
    <div class="no-padding text-right chart-btn"><button type="button" class="umg__button btn" (click)="exportService.download(trackService.artistName + ' - ' + trackService.trackName + ' - ' + 'TrackLineChartOverview', exportService.lineChartOverviewData, exportService.csvTrackLineChartOverview)">Export Data<i class="fa fa-sign-out" aria-hidden="true" style="padding-left: 10px;"></i></button></div>
    <div id="track-detail-line-chart-overview" class="track-detail-line-chart-overview"></div>`,
    styles: [`
      /deep/ #track-detail-line-chart-overview {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #track-detail-line-chart-overview > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class TrackDetailLineChartOverviewComponent implements OnInit {
    ...
    private dailyMediaTypeAudioStreams: any;
    private dailyMediaTypeVideoStreams: any;
    ...
    constructor(public trackService: TrackDetailService,
      ...
      private globals: Globals) {
        ...
        this.filterEmit = fs.TestServiceEmit.subscribe(
            event => {
				this.loadData();
				d3.select(this.elem).select(".track-detail-line-chart-overview svg").remove();
            },
            err => console.log(err),
            () => {
    ...
    loadData() {
        ...
        this.subscriptionLineChartOverview = this.trackService.getLineChartOverviewData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                const dateForm = this.fs.activeParams.date[0] ? this.fs.activeParams.date[0] : [];
                this.dailyMediaTypeData = tempData.track;
                ...
                this.dailyMediaTypeData = chain({}).merge(
                    chain(this.dailyMediaTypeData.totals)
                        ...
                        .map(({ adjustedUnits, units, interval }) => ({ all: units.all, streams: units.streams, audioStreams: units.audioStreams, videoStreams: units.videoStreams, digitalTracks: units.digitalTracks, adjustedAll: adjustedUnits.all, adjustedStreams: adjustedUnits.streams, adjustedAudioStreams: adjustedUnits.audioStreams, adjustedVideoStreams: adjustedUnits.videoStreams, adjustedDigitalTracks: adjustedUnits.digitalTracks, date: interval.date.year + "/" + interval.date.month + "/" + interval.date.day, dateMonth: interval.date.year + "/" + interval.date.month, dateWeek: interval.date.year + "/" + interval.date.week, dateQuarter: interval.date.year + "/" + interval.date.quarter, year: interval.date.year, week: interval.date.week, quarter: interval.date.quarter }))
                        .value(),
                )
                    .values()
                    .flatten()
                ...
                switch (dateForm.period) {
                    case "Year":
						...
						this.dailyMediaTypeAudioStreams = _(this.dailyMediaTypeData)
							.groupBy('dateMonth')
							.map((v, k) => ({
								date: k,
								dateMain: v[0].date,
								dateMonth: v[0].dateMonth,
								units: _.sumBy(v, 'audioStreams'),
								adjustedUnits: _.sumBy(v, 'adjustedAudioStreams'),
								mediaType: "Audio Streams"
							})).value();

						this.dailyMediaTypeVideoStreams = _(this.dailyMediaTypeData)
							.groupBy('dateMonth')
							.map((v, k) => ({
								date: k,
								dateMain: v[0].date,
								dateMonth: v[0].dateMonth,
								units: _.sumBy(v, 'videoStreams'),
								adjustedUnits: _.sumBy(v, 'adjustedVideoStreams'),
								mediaType: "Video Streams"
							})).value();
                ...
                this.dailyMediaTypeAudioStreams = d3.nest().key(function(d: any) { return d.mediaType; }).entries(this.dailyMediaTypeAudioStreams);

                this.dailyMediaTypeVideoStreams = d3.nest().key(function(d: any) { return d.mediaType; }).entries(this.dailyMediaTypeVideoStreams);

                this.dailyMediaTypeDigitalTracks = d3.nest().key(function(d: any) { return d.mediaType; }).entries(this.dailyMediaTypeDigitalTracks);

                this.dailyMediaTypeData = this.dailyMediaTypeAll.concat(this.dailyMediaTypeStreams).concat(this.dailyMediaTypeDigitalTracks);//.concat(this.dailyMediaTypeVideoStreams).concat(this.dailyMediaTypeAudioStreams)
    ...
    lineChartOverview() {
		...
		this.svg = d3.select(this.elem).select('.track-detail-line-chart-overview')
      ...
      .attr("viewBox", "0 0 " + width + " " + height)
		...
		let color = d3.scaleOrdinal()
			.range(["#148baf", "#1bafea", "#6bd4ec"]); //, "#a6d9e8" //"#ddf4f7"
