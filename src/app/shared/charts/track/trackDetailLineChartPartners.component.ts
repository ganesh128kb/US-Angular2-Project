...
@Component({
    selector: 'track-detail-line-chart-partners',
    template: `
    <div class="no-padding text-right chart-btn"><button type="button" class="umg__button btn" (click)="exportService.download(trackService.artistName + ' - ' + trackService.trackName + ' - ' + 'TrackLineChartStreamsPartner', exportService.lineChartPartnerStreamsData, exportService.csvTrackLineChartStreamsPartner)">Export Data<i class="fa fa-sign-out" aria-hidden="true" style="padding-left: 10px;"></i></button></div>
    <div id="track-detail-line-chart-partners" class="track-detail-line-chart-partners"></div>`,
    styles: [`
      /deep/ #track-detail-line-chart-partners {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #track-detail-line-chart-partners > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class TrackDetailLineChartPartnersComponent implements OnInit {
      ...
      private globals: Globals) {
        ...
        this.filterEmit = fs.TestServiceEmit.subscribe(
            ...
            () => {
				...
				d3.select(this.elem).select(".track-detail-line-chart-partners svg").remove();
			});
    ...
    loadData() {
        ...
        this.subscriptionLineChartPartners = this.trackService.getLineChartPartnersData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.dailyPartnerData = tempData.track;
    ...
    lineChartStreams() {
		...
		this.svg = d3.select(this.elem).select('.track-detail-line-chart-partners')
      ...
      .attr("viewBox", "0 0 " + width + " " + height)
        ...
        if (this.data.length < 0) {
            this.svg = d3.select(this.elem).select('.track-detail-line-chart-partners').append('svg')
                .attr('width', this.w)
                .attr('height', '200px')
                .append('text')
