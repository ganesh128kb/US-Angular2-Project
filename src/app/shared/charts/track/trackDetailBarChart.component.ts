...
@Component({
    selector: 'track-detail-bar-chart',
    template: `
    <div class="row">
    ...
    <div id="track-detail-bar-chart" class="track-detail-bar-chart"></div>
    `,
    styles: [`
      /deep/ #track-detail-bar-chart {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #track-detail-bar-chart > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class TrackDetailBarChartComponent implements OnInit {
    ...
    constructor(public trackService: TrackDetailService,
      ...
      private globals: Globals) {
        ...
        this.trackService.addChangeCallBack(() => {
			this.loadData();
		});
    ...
    selectCountry(country: any) {
        ...
        this.trackService.selectCountry(country);
        // console.log(country);
        this.trackService.changeCallBack();
    }

    loadData() {
        this.ready = false;
        d3.select(this.elem).select('.track-detail-bar-chart svg').remove();
        let tempData;

        this.eightWeekRegionData = [];
        this.data = [];

        this.subscriptionWeekRegion = this.trackService.getEightWeekRegionData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.selectedCountry = this.trackService.selectedCountry.name;
                // console.log('selectedCountry', this.selectedCountry);
                this.countryNames = this.trackService.countryNames;
                this.eightWeekRegionData = tempData.track;
                ...
                this.eightWeekRegionData = chain({}).merge(
                    chain(this.eightWeekRegionData.weekly)
                        .filter(tot => !!tot.territory && tot.territory.id === this.trackService.selectedCountry.id)
    ...
    barChart() {
        ...
        this.svg = d3.select(this.elem).select('.track-detail-bar-chart')
