...
@Component({
    selector: 'project-detail-pie-chart',
    template: `
    <div class="row">
    ...
    <div class="col-12 text-center">
    <div id="project-detail-pie-chart" class="project-detail-pie-chart"></div>
    </div>
    </div>
    `,
...
export class ProjectDetailPieChartComponent implements OnInit {
    ...
    public projectCountriesTopFive: any;
    ...
    loadData() {
        let tempData;
        d3.select(this.elem).select(".project-detail-pie-chart svg").remove();
        ...
        this.topFiveCountryData = this.projectService.getCountryData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.topFiveCountryData = tempData.project;
                ...
                this.projectCountriesTopFive = this.topFiveCountryData.slice(0, 5);
                this.projectCountriesTopFive = chain({}).merge(
                    chain(this.projectCountriesTopFive)
                        .filter(tot => !!tot.territory)
                ...
                this.data = this.projectCountriesTopFive;
    ...
    pieChart() {
        ...
        this.svg = d3.select(this.elem).select(".project-detail-pie-chart").append("svg")
        .attr("width", this.w)
        .attr("height", this.h)
        .style("display", "block")
        .style("margin", "0 auto")
