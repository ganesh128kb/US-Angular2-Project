...
@Component({
    selector: 'project-detail-bar-chart',
    template: `
    <div class="row">
    ...
    <div id="project-detail-bar-chart" class="project-detail-bar-chart"></div>
    `,
    styles: [`
      /deep/ #project-detail-bar-chart {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #project-detail-bar-chart > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class ProjectDetailBarChartComponent implements OnInit {
    ...
    constructor(public projectService: ProjectDetailService,
        ...
        this.projectService.addChangeCallBack(() => {
			this.loadData();
		});
    ...
    selectCountry(country: any) {
        ...
        this.projectService.selectCountry(country);
        // console.log(country);
        this.projectService.changeCallBack();
    }

    loadData() {
        this.ready = false;
        d3.select(this.elem).select('.project-detail-bar-chart svg').remove();
        let tempData;

        this.eightWeekRegionData = [];
        this.data = [];

        this.subscriptionWeekRegion = this.projectService.getEightWeekRegionData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                this.selectedCountry = this.projectService.selectedCountry.name;
                // console.log('selectedCountry', this.selectedCountry);
                this.countryNames = this.projectService.countryNames;
                this.eightWeekRegionData = tempData.project;
                // console.log('trackEightWeekChartBefore', this.eightWeekRegionData);
                ...
                this.eightWeekRegionData = chain({}).merge(
                    chain(this.eightWeekRegionData.weekly)
                        .filter(tot => !!tot.territory && tot.territory.id === this.projectService.selectedCountry.id)
                ...
                // console.log('trackEightWeekChart', this.eightWeekRegionData);
    ...
    barChart() {
        ...
        this.svg = d3.select(this.elem).select('.project-detail-bar-chart')
