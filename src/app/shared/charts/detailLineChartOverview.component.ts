...
@Component({
    selector: 'detail-line-chart-overview',
    template: `<div class="error"></div>
    <div class="no-padding text-right chart-btn"><button type="button" class="umg__button btn" (click)="exportService.download(artistService.artistName + ' - ' + 'ArtistLineChartOverview', exportService.lineChartOverviewData, exportService.csvArtistLineChartOverview)">Export Data<i class="fa fa-sign-out" aria-hidden="true" style="padding-left: 10px;"></i></button></div>
    <div id="detail-line-chart-overview" class="chart-overview"></div>`,
    styles: [`
      /deep/ #detail-line-chart-overview {
        position: relative;
        height: 0;
        width: 100%;
        padding: 0;
      ...
      /deep/ #detail-line-chart-overview > svg {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
...
export class DetailLineChartOverviewComponent implements OnInit {
    ...
    constructor(public artistService: ArtistDetailService,
        ...
        this.filterEmit = fs.TestServiceEmit.subscribe(
            event => {
				this.loadData();
				d3.select(this.elem).select(".chart-overview svg").remove();
    ...
    loadData() {
        ...
        this.subscriptionLineChartOverview = this.artistService.getLineChartOverviewData(this.ID).subscribe(
            res => tempData = res,
            err => console.log(err),
            () => {
                const dateForm = this.fs.activeParams.date[0] ? this.fs.activeParams.date[0] : [];
                this.dailyMediaTypeData = tempData.artist;
    ...
    lineChartOverview() {
		...
		this.svg = d3.select(this.elem).select('.chart-overview')
